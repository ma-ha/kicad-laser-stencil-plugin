...
def check_for_bom_button():
    ...
    while True:
        ...
        if button_wx_item_id == 0 or not top_tb.FindTool(button_wx_item_id):
            ...
            top_tb.AddTool(button_wx_item_id, "LStencil", bm,
                           "Generate Laser Stencil GCode", wx.ITEM_NORMAL)
            top_tb.Bind(wx.EVT_TOOL, callback, id=button_wx_item_id)
            top_tb.Realize()


if not os.environ.get('LASER_STENCIL_CLI_MODE', False):
    from .ecad.kicad import LaserStencilPlugin

    plugin = LaserStencilPlugin()
    plugin.register()
