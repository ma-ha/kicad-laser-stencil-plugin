...
class LaserStencilPlugin(pcbnew.ActionPlugin, object):

    def __init__(self):
        super(LaserStencilPlugin, self).__init__()
        self.name = "Generate Laser Stencil GCode"
        self.category = "Read PCB"
        self.pcbnew_icon_support = hasattr(self, "show_toolbar_button")
        self.show_toolbar_button = True
        icon_dir = os.path.dirname(os.path.dirname(__file__))
        self.icon_file_name = os.path.join(icon_dir, 'icon.png')
        self.description = "Generate Laser Stencil GCode"
    ...
    def Run(self):
        ...
        logger = stencil.Logger()
        if not pcb_file_name:
            logger.error('Please save the board file before generating GCode.')
            return

        parser = PcbnewParser(pcb_file_name, config, logger, board)
        try:
            stencil.run_with_dialog(parser, config, logger)
